...
class Mainwindow:
    ...
    def Dashboard(self):
        ...
        self.LoadData()
        self.root.protocol("WM_DELETE_WINDOW",self.clogin.destroy)
    ...
    def Form(self,index=None,account=None):
        ...
        if account:
            ...
            save_butt=Button(form,text="Save",padx=20,pady=10,bg=self.accent,fg="white",relief=FLAT,command=lambda:self.vault.update_account(index,web_entry.get(),user_entry.get(),pasw_entry.get()))

        else:
            save_butt=Button(form,text="Save",padx=20,pady=10,bg=self.accent,fg="white",relief=FLAT,command=lambda:self.vault.add_account(web_entry.get(),user_entry.get(),pasw_entry.get()))
